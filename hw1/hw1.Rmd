...
The dimensions of 'my_df' are the rows and the columns. The rows show the person's name and his/her corresponding preference of social media. The colum on the left lists all the names and the colum on the right lists all the social media platforms.
...
```{r}
print(table(my_df$Preference))
```


e) Write R code that returns what Gisele prefers.
```{r}
print(table(my_df$Preference,my_df$Person))
```
